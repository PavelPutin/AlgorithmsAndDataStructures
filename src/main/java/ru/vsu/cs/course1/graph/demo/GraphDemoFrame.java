...
import ru.vsu.cs.pavel_p_a.task7.Solution;
...
public class GraphDemoFrame extends JFrame {
    ...
    private JTable tableEntrances;
    private JTable tableExits;
    ...
    public GraphDemoFrame() {
        ...
        JTableUtils.initJTableForArray(tableEntrances, DEFAULT_CELL_SIZE, false, false, true, true);
        JTableUtils.initJTableForArray(tableExits, DEFAULT_CELL_SIZE, false, false, true, true);
        ...
        buttonCreateGraph.addActionListener(e -> {
            try {
                ...
                int[] entrances = JTableUtils.readIntArrayFromJTable(tableEntrances);
                int[] exits = JTableUtils.readIntArrayFromJTable(tableExits);
                int maxQuantityOfPeople = Solution.findMaxQuantityOfPeopleEdge(graph, entrances, exits);

                showSystemOut(() -> {
                    System.out.println(maxQuantityOfPeople);
                });
            } catch (Exception exc) {
                SwingUtils.showErrorMessageBox(exc);
            }
